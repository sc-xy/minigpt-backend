...
import com.xy.springboot.mapper.UserMapper;
import com.xy.springboot.model.entity.User;
import com.xy.springboot.model.enums.UserRoleEnum;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    @Override
    public User checkOldPassword(Long userId, String userAccount, String oldPassword) {
        // 1. 校验
        if (userId == null || StringUtils.isAnyBlank(userAccount, oldPassword)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "参数为空");
        }
        // 2. 加密
        ...
        // 用户不存在
        if (user == null) {
            log.info("user check old password failed, userAccount cannot match userPassword");
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户不存在或密码错误");
        }
        return user;
    }
